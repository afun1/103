...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.log(`🎯 Loading all videos for ${normalizedEmail} from folder ${folderId}`);
        ...
        console.log(`✅ Returning ${userVideos.length} recordings for ${normalizedEmail}`);
        return res.status(200).json(userVideos);
    } catch (error) {
        console.error('❌ Failed to load user videos (legacy route):', error);
